...
export async function updateInvoiceItems(
  ...
  // Load current invoice to get existing vat_rate if not provided
  const { data: existing, error: fetchErr } = await db()
    ...
    .eq('user_id', uid())
    .single();
  if (fetchErr) throw new Error(`Rēķins nav atrasts: ${fetchErr.message}`);

  const effectiveVatRate = vatRate ?? existing.vat_rate ?? 0;
  ...
  const subtotal = Math.round(mappedItems.reduce((s, i) => s + i.total, 0) * 100) / 100;
  ...
  if (delErr) throw new Error(`Kļūda dzēšot pozīcijas: ${delErr.message}`);

  const { error: insErr } = await db().from('invoice_items').insert(mappedItems);
  if (insErr) throw new Error(`Kļūda ievietojot pozīcijas: ${insErr.message}`);

  // Update invoice totals
  const { data, error: updErr } = await db()
    .from('invoices')
    .update({ subtotal, vat_rate: effectiveVatRate, vat_amount: vatAmount, total })
    .eq('id', invoiceId)
    .eq('user_id', uid())
    .select()
    .single();
